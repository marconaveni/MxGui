...
struct MxIconManager
{
    ...
    void loadFromMemory(const std::string& fontNameID, const unsigned char* fileData, int dataSize, int fontSize, const int* codepoints, int codepointCount, bool smooth)
    {
        ...
        m_icons.insert_or_assign(fontNameID, fontData);
    }

    void unload()
    {
        for (auto& [idFontName, fontDatas] : m_icons)
        {
    ...
    {
        const MxFont* font = getIcon(fontNameID, fontSize);
        return measureTextInternal(*font, text, fontSize, spacing);
    }

    const MxFont* getIcon(const std::string& fontNameID, int size)
    {
        static MxFont empty{};

        auto itFontData = m_icons.find(fontNameID);
        if (itFontData != m_icons.end())
        {
    ...
    std::unordered_map<std::string, MxFontData> m_icons{};
};
...
void drawIconEx(int codepoint, MxVec2 position, MxColor color, int size)
{
    ...
    const MxFont* font = getCore().iconManager.getIcon(MX_FONT_AWESOME_ID, size);
